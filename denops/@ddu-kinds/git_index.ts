...
function getRepositoryRoot(cwd: string): string {
  const cmd = new Deno.Command("git", { args: ["rev-parse", "--show-toplevel"], cwd: cwd })
...
export class Kind extends BaseKind<Params> {
  actions: Record<string, (args: ActionArguments<Params>) => Promise<ActionFlags>> = {
    restoreStaged: async (args: ActionArguments<Params>): Promise<ActionFlags> => {
      ...
      for (const item of args.items) {
        ...
        const cmd = new Deno.Command("git", { args: ["restore", "--staged", action.path], cwd: cwd });
        const result = cmd.outputSync();
